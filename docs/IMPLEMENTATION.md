...
└── fitting/               §8
    ...
    ├── glide.py           BatchedGlideRender            (many tensions at once)
...
444 tests, ~3 min. Nine files (`test_data_integrity.py` is heavily parametrized — one case per manifest per check):
